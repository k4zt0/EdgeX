...
| **통합 스위치** | 없음 (대상의 주소를 씀) | **화면에 있는 요소를 골라 하나로 조작합니다.** 아래 절 참조 |
...
요소마다 카드를 깔지 않고 **한 장으로 화면 편집에 있는 기능을 모두 수행**하려면 이 형식을 씁니다.

```
┌──────────────────────────────────────┐
│ job start · ON  +2            M01009 │  ← 제목도 값에 따라 바뀜
│ [ job start · M01009              ▾ ]│  ← 대상 고르기
│ ┌──────────────────────────────────┐ │
│ │ ● job start · ON                 │ │  ← 지금 도는 것 전부
│ │   M01009 : ON   ·   P00121 : ON  │ │  ← 디바이스 · 상태확인 값
│ │ ● 실행 중 · 펌프 순간 · 순간       │ │
│ │   M01010 : ---  ·   P00122 : ON  │ │
│ │ ● 밸브 개방 · 오류                │ │  ← 실패는 빨간색
...
│ └──────────────────────────────────┘ │
│ [           토글                    ]│
│ [   M01009 : ON  ·  P00121 : ON     ]│  ← 버튼에도 값이 뜸
└──────────────────────────────────────┘
```

* **대상** — 주소가 있는 요소가 모두 목록에 뜹니다. 화면 편집에서 요소를 늘리거나 이름을 바꾸면
  목록도 따라가고, 고르고 있던 대상은 그대로 남습니다.
* **버튼** — 고른 대상의 `스위치 동작` 을 그대로 씁니다. 글자도 `토글 / ON / OFF / 순간` 으로 바뀌고,
  아래줄에 **디바이스와 상태확인 디바이스의 현재 값**이 함께 뜹니다.
  숫자입력을 고르면 값 입력칸이 함께 나오고 버튼은 `쓰기` 가 됩니다.
  램프·숫자표시처럼 조작할 수 없는 대상은 `표시 전용` 으로 잠깁니다.

**실행 중인 것은 하나만이 아니라 전부** 한 줄씩 뜹니다. 화면의 다른 카드에서 누른 조작도 함께 나옵니다.

| 줄에 뜨는 것 | 색 |
|---|---|
| 쓰기·토글이 도는 중 (`실행 중 · 이름 · 동작`) | 초록 |
| 비트가 켜져 있는 요소 (`이름 · ON`) | 초록 |
| 조작이 제대로 안 된 요소 (`이름 · 오류`) | **빨강** — 마우스를 올리면 원인 |

줄마다 아래에 `디바이스 : 값 · 상태확인 디바이스 : 값` 이 붙습니다.
**줄이 늘면 카드도 그만큼 세로로 커집니다.** (배치에서 정한 높이보다 작아지지는 않습니다)
제목에는 첫 줄과 나머지 개수(`+2`)가 나옵니다.
