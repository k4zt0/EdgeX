...
* **운전 화면** — 카드형 HMI. 상태는 색으로 표시되고, ON 인 요소는 모든 테마에서 발광합니다(아래 *라이팅* 참조).
  도면(작화 영역)이 테두리로 표시되고 그 안팎을 자유롭게 스크롤합니다. 도면 크기는 툴바의 `화면 크기` 로 바꿉니다(기본 1600×1000).
...
| 상태 | 표시 |
|---|---|
| 스위치 ON | 지금 상태에 해당하는 버튼이 초록색(`On`)으로 점등됩니다. `ON/OFF` 쌍은 ON 쪽이, 토글은 버튼 자체가 켜집니다. |
| 스위치 OFF | `ON/OFF` 쌍에서 OFF 쪽이 회색(`Off`)으로 점등되어 지금 상태를 알려 줍니다. |
...
> 이동·크기조절은 **배치 편집이 켜져 있을 때만** 됩니다(PLC 오조작 방지). 툴바 버튼에 `배치 편집 : 켬 / 끔` 이 그대로 표시되고, 운전 화면 위 안내줄도 상태에 따라 바뀝니다.
...
* **도면 크기** — 툴바 `화면 크기 [폭] × [높이]` (640×480 ~ 20000×20000). 프로젝트 XML에 `ScreenWidth` / `ScreenHeight` 로 저장되며, 이 항목이 없는 v6 파일은 기본값 1600×1000으로 열립니다.
