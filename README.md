...
화면에 있는 요소를 **한 장에서 지켜보는** 판입니다. 값을 켜는 조작은 하지 않고, 끄는 것만 합니다.
...
* 화면 편집에 있는 요소가 **전부** 뜹니다. 급한 것부터 위로 옵니다 — 오류 → 실행 중 → 켜짐 → 나머지.
* 줄마다 아래에 `디바이스 : 값 · 상태확인 디바이스 : 값` 이 붙습니다.
* 화면 편집에서 요소를 늘리거나 이름을 바꾸면 목록도 따라갑니다.
* 줄이 늘면 카드도 세로로 커집니다. 배치에서 정한 높이보다 작아지지 않고,
  너무 길어지면 더 키우지 않고 목록 안에서 스크롤합니다.
...
* 누르면 출력 창에 `KILL SWITCH — 비트 n개를 OFF 로 씁니다` 가 남고,
  각 쓰기가 목록에 `실행 중` 으로 하나씩 나타납니다.
...
tests/XgbHmi.Tests/   가짜 XGT 서버로 프레임까지 검증하는 테스트 54개
