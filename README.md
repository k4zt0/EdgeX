...
┌─────────────────────────────┐
│ 통합 스위치                  │
│ [ job start · M01009    ▾ ] │  ← 대상 고르기 (화면 편집에 있는 요소 전부)
│ ●  job start · ON           │  ← 대상 상태 / 지금 도는 조작
│ [        토글        ]      │  ← 대상의 동작으로 조작
└─────────────────────────────┘
```
...
* **버튼** — 고른 대상의 `스위치 동작` 을 그대로 씁니다. 글자도 `토글 / ON / OFF / 순간` 으로 바뀝니다.
  숫자입력을 고르면 값 입력칸이 함께 나오고 버튼은 `쓰기` 가 됩니다.
  램프·숫자표시처럼 조작할 수 없는 대상은 버튼이 `표시 전용` 으로 잠깁니다.
* **램프** — 대상이 램프면 램프 상태를 그대로 띄웁니다.

조작이 돌면 **어느 조작이 도는지** 이 한 장에 나옵니다. 화면의 다른 카드에서 누른 조작도 여기에 뜹니다.

| 상태 | 표시 |
|---|---|
| 실행 중 · 정상 | 초록색 점등 — `실행 중 · 이름 · 동작` |
| 끝남 · 정상 | 초록색 점등 — `이름 · 동작` |
| **제대로 실행 안 됨** | **빨간색 점등** — `이름 · 오류`, 카드에 마우스를 올리면 원인 |
